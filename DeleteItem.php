...
if (isset($_GET["id"])) {
    ...
    $cup = cup.$userId;
    $query = "SELECT ItemName FROM `{$cup}` WHERE ID = '$safeId' LIMIT 1";
    $item = mysqli_fetch_assoc(mysqli_query($dbconnect, $query));
    if (!empty($item)) {
        if (isset($_POST["Delete"])) {
            $delete = "DELETE FROM `{$cup}` WHERE `ID` = '$safeId'";
    ...
    <body>
        <div id="Holder">
            ...
            <div id="Content">
                ...
                <div id="ContentRight">
                    <h6><br><?php if (!isset($_POST["Delete"])) { echo "Do you really want to delete <b>".$item['ItemName']."</b> from your cupboard?";} echo $dbErr1;echo $dbErr2; echo $dbsuccess?></h6><br />
                    <form method="POST" action="DeleteItem.php?id=<?php echo $safeId ?>">
                        <input type="submit" name="Delete" value="Delete Item"><h6><br />or<a href="ViewItem.php">Return to Cupboard</h6>
                    </form>
                </div>
            </div>
            <div id="Footer"><br /><br/>
